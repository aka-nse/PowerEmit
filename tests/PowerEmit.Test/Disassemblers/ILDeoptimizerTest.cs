...
{
    private static (IILStreamAction input, IILStreamAction deoptimized)[] TestCasesBase()
    {
    ...
    {
        var data = new TheoryData<IILStreamAction, IILStreamAction>();
        foreach(var (input, deoptimized) in TestCasesBase())
        {
            data.Add(input, deoptimized);
        }
        return data;
    }
    [Theory]
    ...
    [Fact]
    public void DeoptimizeStream()
    {
        var inputStream = new List<IILStreamAction>();
        var expectedDeoptimizedStream = new List<IILStreamAction>();
        foreach(var (input, deoptimized) in TestCasesBase())
        {
            inputStream.Add(input);
            expectedDeoptimizedStream.Add(deoptimized);
        }
        var actualDeoptimizedStream = ILDeoptimizer.Instance.Deoptimize(inputStream);
        Assert.Equal(expectedDeoptimizedStream, actualDeoptimizedStream);
    }
}
