...
namespace PowerEmit
{
    ...
    {
        /// <summary> Creates new instruction item of <c>brfalse</c>. </summary>
        /// <param name="operand"></param>
        /// <returns></returns>
        public static IILStreamInstruction Brfalse(LabelDescriptor operand)
            => new Emit_Brfalse(operand);


        private sealed class Emit_Brfalse : ILStreamInstruction<LabelDescriptor>
        {
            public override OpCode OpCode => OpCodes.Brfalse;

            public Emit_Brfalse(LabelDescriptor operand)
                : base(operand)
            {
            }
