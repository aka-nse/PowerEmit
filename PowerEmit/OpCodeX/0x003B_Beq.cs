...
namespace PowerEmit
{
    ...
    {
        /// <summary> Creates new instruction item of <c>beq</c>. </summary>
        /// <param name="operand"></param>
        /// <returns></returns>
        public static IILStreamInstruction Beq(LabelDescriptor operand)
            => new Emit_Beq(operand);


        private sealed class Emit_Beq : ILStreamInstruction<LabelDescriptor>
        {

            public override OpCode OpCode => OpCodes.Beq;

            public Emit_Beq(LabelDescriptor operand)
                : base(operand)
            {
            }
