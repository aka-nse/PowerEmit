...
namespace PowerEmit
{
    ...
    {
        /// <summary> Creates new instruction item of <c>shr</c>. </summary>
        /// <returns></returns>
        public static IILStreamInstruction Shr()
            => new Emit_Shr();


        private sealed class Emit_Shr : ILStreamInstruction
        {
            ...
            public override void Invoke(IILInvocationState state)
            {
                ...
                {
                    (StackValue.Int32     x, StackValue.Int32     y) => StackValue.FromValue(x.Value >> (int)y.Value),
                    (StackValue.Int32     x, StackValue.NativeInt y) => StackValue.FromValue(x.Value >> (int)y.Value),
                    (StackValue.Int64     x, StackValue.Int32     y) => StackValue.FromValue(x.Value >> (int)y.Value),
                    (StackValue.Int64     x, StackValue.NativeInt y) => StackValue.FromValue(x.Value >> (int)y.Value),
                    (StackValue.NativeInt x, StackValue.Int32     y) => StackValue.FromValue(x.Value >> (int)y.Value),
                    (StackValue.NativeInt x, StackValue.NativeInt y) => StackValue.FromValue(x.Value >> (int)y.Value),
                    _ => throw new Exception(),
                };
