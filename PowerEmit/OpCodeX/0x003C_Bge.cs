...
namespace PowerEmit
{
    ...
    {
        /// <summary> Creates new instruction item of <c>bge</c>. </summary>
        /// <param name="operand"></param>
        /// <returns></returns>
        public static IILStreamInstruction Bge(LabelDescriptor operand)
            => new Emit_Bge(operand);


        private sealed class Emit_Bge : ILStreamInstruction<LabelDescriptor>
        {
            public override OpCode OpCode => OpCodes.Bge;

            public Emit_Bge(LabelDescriptor operand)
                : base(operand)
            {
            }
