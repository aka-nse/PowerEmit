        ...
        private sealed class Emit_Conv_Ovf_U1_Un : ILStreamInstruction
        {
            ...
            public override void Invoke(IILInvocationState state)
            {
                checked
                {
                    ...
                    {
                        StackValue.Int32       x => StackValue.FromValue((byte)unchecked((uint) x.Value)),
                        StackValue.Int64       x => StackValue.FromValue((byte)unchecked((ulong)x.Value)),
                        StackValue.NativeInt   x => StackValue.FromValue((byte)unchecked((nuint)x.Value)),
                        StackValue.F x => StackValue.FromValue((byte)Math.Abs(Math.Truncate(x.Value))),
                        _ => throw new Exception(),
                    };
