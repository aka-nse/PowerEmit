...
namespace PowerEmit
{
    ...
    {
        /// <summary> Creates new instruction item of <c>bge.un</c>. </summary>
        /// <param name="operand"></param>
        /// <returns></returns>
        public static IILStreamInstruction Bge_Un(LabelDescriptor operand)
            => new Emit_Bge_Un(operand);


        private sealed class Emit_Bge_Un : ILStreamInstruction<LabelDescriptor>
        {
            public override OpCode OpCode => OpCodes.Bge_Un;

            public Emit_Bge_Un(LabelDescriptor operand)
                : base(operand)
            {
            }
