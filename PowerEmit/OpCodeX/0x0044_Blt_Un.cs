...
namespace PowerEmit
{
    ...
    {
        /// <summary> Creates new instruction item of <c>blt.un</c>. </summary>
        /// <param name="operand"></param>
        /// <returns></returns>
        public static IILStreamInstruction Blt_Un(LabelDescriptor operand)
            => new Emit_Blt_Un(operand);


        private sealed class Emit_Blt_Un : ILStreamInstruction<LabelDescriptor>
        {
            public override OpCode OpCode => OpCodes.Blt_Un;

            public Emit_Blt_Un(LabelDescriptor operand)
                : base(operand)
            {
            }
