    ...
    partial class OpCodeX
    {
        ...
        private sealed class Emit_Conv_R4 : ILStreamInstruction
        {
            ...
            public override void Invoke(IILInvocationState state)
            {
                ...
                {
                    StackValue.IInt32       x => StackValue.FromValue((float)x.Value),
                    StackValue.IInt64       x => StackValue.FromValue((float)x.Value),
                    StackValue.INativeInt   x => StackValue.FromValue((float)x.Value),
                    StackValue.IFloat x => StackValue.FromValue((float)x.Value),
                    _ => throw new Exception(),
                };
