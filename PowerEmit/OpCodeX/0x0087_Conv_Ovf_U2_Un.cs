        ...
        private sealed class Emit_Conv_Ovf_U2_Un : ILStreamInstruction
        {
            ...
            public override void Invoke(IILInvocationState state)
            {
                checked
                {
                    ...
                    {
                        StackValue.IInt32     x => StackValue.FromValue((ushort)unchecked((uint) x.Value)),
                        StackValue.IInt64     x => StackValue.FromValue((ushort)unchecked((ulong)x.Value)),
                        StackValue.INativeInt x => StackValue.FromValue((ushort)unchecked((nuint)x.Value)),
                        StackValue.IFloat x => StackValue.FromValue((ushort)Math.Abs(Math.Truncate(x.Value))),
                        _ => throw new Exception(),
                    };
