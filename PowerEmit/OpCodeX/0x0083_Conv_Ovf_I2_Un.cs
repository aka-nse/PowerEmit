        ...
        private sealed class Emit_Conv_Ovf_I2_Un : ILStreamInstruction
        {
            ...
            public override void Invoke(IILInvocationState state)
            {
                checked
                {
                    ...
                    {
                        StackValue.Int32       x => StackValue.FromValue((short)unchecked((uint) x.Value)),
                        StackValue.Int64       x => StackValue.FromValue((short)unchecked((ulong)x.Value)),
                        StackValue.NativeInt   x => StackValue.FromValue((short)unchecked((nuint)x.Value)),
                        StackValue.F x => StackValue.FromValue((short)Math.Abs(Math.Truncate(x.Value))),
                        _ => throw new Exception(),
                    };
