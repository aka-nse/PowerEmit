        ...
        private sealed class Emit_Conv_Ovf_U8_Un : ILStreamInstruction
        {
            ...
            public override void Invoke(IILInvocationState state)
            {
                checked
                {
                    ...
                    {
                        StackValue.IInt32     x => StackValue.FromValue((ulong)unchecked((uint) x.Value)),
                        StackValue.IInt64     x => StackValue.FromValue((ulong)unchecked((ulong)x.Value)),
                        StackValue.INativeInt x => StackValue.FromValue((ulong)unchecked((nuint)x.Value)),
                        StackValue.IFloat x => StackValue.FromValue((ulong)Math.Abs(Math.Truncate(x.Value))),
                        _ => throw new Exception(),
                    };
