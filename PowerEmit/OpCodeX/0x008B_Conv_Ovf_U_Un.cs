        ...
        private sealed class Emit_Conv_Ovf_U_Un : ILStreamInstruction
        {
            ...
            public override void Invoke(IILInvocationState state)
            {
                checked
                {
                    ...
                    {
                        StackValue.Int32       x => StackValue.FromValue((nuint)unchecked((uint) x.Value)),
                        StackValue.Int64       x => StackValue.FromValue((nuint)unchecked((ulong)x.Value)),
                        StackValue.NativeInt   x => StackValue.FromValue((nuint)unchecked((nuint)x.Value)),
                        StackValue.F x => StackValue.FromValue((nuint)Math.Abs(Math.Truncate(x.Value))),
                        _ => throw new Exception(),
                    };
