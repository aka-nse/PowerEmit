        ...
        private sealed class Emit_Conv_U8 : ILStreamInstruction
        {
            ...
            public override void Invoke(IILInvocationState state)
            {
                unchecked
                {
                    ...
                    {
                        StackValue.Int32       x => StackValue.FromValue((ulong)x.Value),
                        StackValue.Int64       x => StackValue.FromValue((ulong)x.Value),
                        StackValue.NativeInt   x => StackValue.FromValue((ulong)x.Value),
                        StackValue.F x => StackValue.FromValue((ulong)x.Value),
                        _ => throw new Exception(),
                    };
