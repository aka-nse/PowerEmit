...
namespace PowerEmit
{
    ...
    {
        public static IILStreamOpCode Ldloc_Opt(LocalDescriptor local)
            => new Emit_Ldloc_Opt(local);


        private sealed class Emit_Ldloc_Opt : IILStreamOpCode
        {
            ...
            internal Emit_Ldloc_Opt(LocalDescriptor local)
            {
                Local = local;
            }
